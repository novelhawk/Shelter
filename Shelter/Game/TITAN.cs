...
public class TITAN : Photon.MonoBehaviour
{
    ...
    public void StartLaughing()
    {
        if (!this.hasDie && this.abnormalType != AbnormalType.Crawler)
        {
            ...
            {
                photonView.RPC(Rpc.Laugh, PhotonTargets.All, 0f);
            }
    ...
    private GameObject checkIfHitCrawlerMouth(Transform headTransform, float rad)
    {
    ...
    private GameObject checkIfHitHand(Transform hand)
    {
        ...
        foreach (Collider c in Physics.OverlapSphere(hand.GetComponent<SphereCollider>().transform.position, num + 1f))
        {
    ...
    private GameObject checkIfHitHead(Transform headTransform, float rad)
    {
        float num = rad * this.myLevel;
        foreach (GameObject obj2 in GameObject.FindGameObjectsWithTag("Player"))
        {
    ...
    public void dieBlow(Vector3 attacker, float hitPauseTime)
    {
        ...
        {
            photonView.RPC(Rpc.Explode, PhotonTargets.All, attacker, hitPauseTime);
        }
    }
    ...
    public void dieBlowFunc(Vector3 attacker, float hitPauseTime)
    {
        if (!this.hasDie)
        {
            ...
            if (photonView.isMine)
            {
                ...
                {
                    this.grabbedTarget.GetPhotonView().RPC(Rpc.netUngrabbed, PhotonTargets.All);
                }
                if (this.nonAI)
                {
    ...
    {
        PhotonView cannonView = PhotonView.Find(viewID);
        if (cannonView != null)
        {
            int damage = 0;
            if (PhotonNetwork.isMasterClient)
            {
                this.OnTitanDie(cannonView);
            }
            if (this.nonAI)
            {
                GameManager.instance.TitanGetKill(cannonView.owner, damage, Player.Self.Properties.Name, null);
            }
            else
            {
                GameManager.instance.TitanGetKill(cannonView.owner, damage, name, null);
            }
        }
        else
        {
            //BUG: cannonView is null on all code paths
            GameManager.instance.photonView.RPC(Rpc.ShowDamage, cannonView.owner, this.speed);
        }
    }
    ...
    public void dieHeadBlow(Vector3 attacker, float hitPauseTime)
    {
        if (this.abnormalType != AbnormalType.Crawler)
        {
            ...
            {
                photonView.RPC(Rpc.HeadBlow, PhotonTargets.All, attacker, hitPauseTime);
            }
        }
    }
            ...
            if (num == 1)
            {
                ...
                foreach (GameObject obj2 in GameObject.FindGameObjectsWithTag("Player"))
                {
                    if (Vector3.Distance(obj2.transform.position, position) < GameManager.settings.ExplodeRadius)
                    {
                        ...
                        obj2.GetComponent<HERO>().photonView.RPC(Rpc.DieRC, PhotonTargets.All, -1, "Server ");
                    }
                }
            }
        }
    ...
    private void FindNearestFacingHero()
    {
        ...
        foreach (HERO hero in this.MultiplayerManager.GetPlayers())
        {
            ...
            float num5 = Vector3.Distance(go.transform.position, position);
            if (num5 < positiveInfinity)
            {
                Vector3 vector2 = go.transform.position - this.baseTransform.position;
        ...
        if (obj2 != null)
        {
            ...
            this.myHero = obj2;
            if (hero != this.myHero && IN_GAME_MAIN_CAMERA.GameType == GameType.Multiplayer && PhotonNetwork.isMasterClient)
            {
                if (this.myHero == null)
                    photonView.RPC(Rpc.SetTarget, PhotonTargets.Others, -1);
                else
                    photonView.RPC(Rpc.SetTarget, PhotonTargets.Others, hero.GetPhotonView().viewID);
            }
            this.tauntTime = 5f;
        }
    }
    ...
    private void FindNeareastHero()
    {
        ...
        if (this.myHero != oldHero && IN_GAME_MAIN_CAMERA.GameType == GameType.Multiplayer && PhotonNetwork.isMasterClient)
        {
            if (this.myHero == null)
                ...
                photonView.RPC(Rpc.SetTarget, PhotonTargets.Others, myHero.GetPhotonView().viewID);
        }
                        ...
                        if (this.state == TitanState.Idle)
                        {
                            ...
                            else if (this.abnormalType == AbnormalType.Crawler)
                            {
                                ...
                                if (obj2 != null)
                                {
                                    ...
                                    {
                                        obj2.GetComponent<HERO>().markDie();
                                        obj2.GetComponent<HERO>().photonView.RPC(Rpc.Die, PhotonTargets.All, 
    ...
    {
        Transform t = this.transform.Find("Amarture/Core/Controller_Body/hip/spine/chest/shoulder_L/upper_arm_L/forearm_L/hand_L/hand_L_001");
        this.grabTF.transform.parent = t;
        this.grabTF.transform.position = t.GetComponent<SphereCollider>().transform.position;
        this.grabTF.transform.rotation = t.GetComponent<SphereCollider>().transform.rotation;
        Transform transform1 = this.grabTF.transform;
        transform1.localPosition -= Vector3.right * t.GetComponent<SphereCollider>().radius * 0.3f;
        Transform transform2 = this.grabTF.transform;
        transform2.localPosition -= Vector3.up * t.GetComponent<SphereCollider>().radius * 0.51f;
        Transform transform3 = this.grabTF.transform;
        transform3.localPosition -= Vector3.forward * t.GetComponent<SphereCollider>().radius * 0.3f;
    ...
    {
        Transform t = this.transform.Find("Amarture/Core/Controller_Body/hip/spine/chest/shoulder_R/upper_arm_R/forearm_R/hand_R/hand_R_001");
        this.grabTF.transform.parent = t;
        this.grabTF.transform.position = t.GetComponent<SphereCollider>().transform.position;
        this.grabTF.transform.rotation = t.GetComponent<SphereCollider>().transform.rotation;
        Transform transform1 = this.grabTF.transform;
                ...
                if (photonView.isMine)
                {
                    ...
                    if (this.abnormalType != AbnormalType.Crawler && this.state != TitanState.Attacking && this.state != TitanState.Down && this.state != TitanState.Hitting && (this.state != TitanState.Recovering && this.state != TitanState.Eating) && this.state != TitanState.Blinded && !this.hasDie && this.myDistance < 100f && this.myHero != null)
                    {
                        ...
                        if (!this.asClientLookTarget)
                        {
                            ...
                            photonView.RPC(Rpc.SetLookTarget, PhotonTargets.Others, true);
                        }
                        flag2 = true;
                    }
                    if (!(flag2 || !this.asClientLookTarget))
                    {
                        ...
                        photonView.RPC(Rpc.SetLookTarget, PhotonTargets.Others, objArray3);
                    }
    ...
    public void HitLeft(Vector3 attacker, float hitPauseTime)
    {
        if (this.abnormalType != AbnormalType.Crawler)
        {
            if (IN_GAME_MAIN_CAMERA.GameType == GameType.Singleplayer)
                ...
                photonView.RPC(Rpc.NeckHitLeft, PhotonTargets.All, attacker, hitPauseTime);
        }
    }
    ...
    public void HitRight(Vector3 attacker, float hitPauseTime)
    {
        if (this.abnormalType != AbnormalType.Crawler)
        {
            if (IN_GAME_MAIN_CAMERA.GameType == GameType.Singleplayer)
                ...
                photonView.RPC(Rpc.NeckHitRight, PhotonTargets.All, attacker, hitPauseTime);
        }
    }
    ...
    private void idle(float idleTime = 0f)
    {
        this.stuck = false;
        this.sbtime = idleTime;
        ...
        if (target != null)
        {
            if (IN_GAME_MAIN_CAMERA.GameType == GameType.Multiplayer && photonView.isMine)
            {
                if (!target.GetComponent<HERO>().HasDied())
                {
                    ...
                    if (this.nonAI)
                    {
                        target.GetComponent<HERO>().photonView.RPC(Rpc.DieRC, PhotonTargets.All, photonView.viewID, name);
                    }
                    else
                    {
                        target.GetComponent<HERO>().photonView.RPC(Rpc.DieRC, PhotonTargets.All, -1, name);
                    }
                }
            }
    ...
    public void loadskin()
    {
        ...
        if ((IN_GAME_MAIN_CAMERA.GameType == GameType.Singleplayer || photonView.isMine) && ModuleManager.Enabled(nameof(ModuleEnableSkins)))
        {
            ...
            if (GameManager.settings.Randomize)
                eteId = titanSkin.Eyes[Random.Range(0, titanSkin.Eyes.Length)];                
            else
                eteId = titanSkin.Eyes[index];
            
            this.skin += index;
            
            if (Utility.IsValidImageUrl(eteId))
                this.eye = true;
            GetComponent<TITAN_SETUP>().setVar(this.skin, this.eye);
            
            if (IN_GAME_MAIN_CAMERA.GameType == GameType.Singleplayer)
                StartCoroutine(this.LoadSkinEnumerator(body, eteId));
            else
                photonView.RPC(Rpc.LoadSkin, PhotonTargets.AllBuffered, body, eteId);
        }
    }

    public IEnumerator LoadSkinEnumerator(string body, string eyeUrl)
    {
        while (!this.hasSpawn)
        {
            yield return null;
        ...
        foreach (Renderer myRenderer in this.GetComponentsInChildren<Renderer>())
        {
            ...
            {
                if (eyeUrl.EqualsIgnoreCase("transparent"))
                {
                    myRenderer.enabled = false;
                }
                else if (Utility.IsValidImageUrl(eyeUrl))
                {
                    if (!GameManager.linkHash[0].ContainsKey(eyeUrl))
                    {
                        unloadAssets = true;
                        ...
                        GameManager.linkHash[0].Add(eyeUrl, myRenderer.material);
                        myRenderer.material = (Material)GameManager.linkHash[0][eyeUrl];
                    }
                    else
                    {
                        myRenderer.material = (Material)GameManager.linkHash[0][eyeUrl];
    ...
    public void setAbnormalType2(AbnormalType type, bool forceCrawler)
    {
        ...
        if (IN_GAME_MAIN_CAMERA.GameType != GameType.Singleplayer && photonView.isMine)
        {
            ...
            photonView.RPC(Rpc.SetAbnormalType, PhotonTargets.AllBuffered, parameters);
        }
        else if (IN_GAME_MAIN_CAMERA.GameType == GameType.Singleplayer)
        {
    ...
    private void Start()
    {
        ...
        {
            photonView.RPC(Rpc.UpdateHealthLabel, PhotonTargets.AllBuffered, this.currentHealth, this.maxHealth);
        }
    ...
    public void suicide()
    {
        ...
        if (this.nonAI)
        {
            GameManager.instance.SendKillInfo(false, string.Empty, true, Player.Self.Properties.Name);
        }
    ...
    public void TitanGetHit(int viewID, int hitSpeed)
    {
        ...
        if (view != null)
        {
            ...
            {
                this.healthTime = Time.time;
                if (hitSpeed >= GameManager.settings.MinimumDamage || this.abnormalType == AbnormalType.Crawler)
                {
                    this.currentHealth -= hitSpeed;
                }
                ...
                if (this.currentHealth < 0f)
                {
                    ...
                    if (this.nonAI)
                    {
                        GameManager.instance.TitanGetKill(view.owner, hitSpeed, Player.Self.Properties.Name, null);
                    }
                    else
                    {
                        GameManager.instance.TitanGetKill(view.owner, hitSpeed, name, null);
                    }
                }
                else
                {
                    GameManager.instance.photonView.RPC(Rpc.ShowDamage, view.owner, hitSpeed);
                }
            }
        }
    }
            ...
            else if (this.state != TitanState.Idle)
            {
                if (this.state == TitanState.Attacking)
                {
                        ...
                        if (this.baseAnimation["attack_" + this.attackAnimation].normalizedTime >= 0.11f && this.baseAnimation["attack_" + this.attackAnimation].normalizedTime <= 0.16f)
                        {
                            ...
                            if (obj5 != null)
                            {
                                ...
                                else if (!(IN_GAME_MAIN_CAMERA.GameType != GameType.Multiplayer || !photonView.isMine || obj5.GetComponent<HERO>().HasDied()))
                                {
                                    ...
                                    obj5.GetComponent<HERO>().photonView.RPC(Rpc.Die, PhotonTargets.All, objArray3);
                                }
                            }
                        }
                        if (this.baseAnimation["attack_" + this.attackAnimation].normalizedTime >= 0.27f && this.baseAnimation["attack_" + this.attackAnimation].normalizedTime <= 0.32f)
                        {
                            ...
                            if (obj6 != null)
                            {
                                ...
                                else if (!(IN_GAME_MAIN_CAMERA.GameType != GameType.Multiplayer || !photonView.isMine || obj6.GetComponent<HERO>().HasDied()))
                                {
                                    ...
                                    obj6.GetComponent<HERO>().photonView.RPC(Rpc.Die, PhotonTargets.All, objArray4);
                                }
                            }
                        }
                    }
                        ...
                        if (this.leftHandAttack)
                        {
                            ...
                            if (obj7 != null)
                            {
                                ...
                                else if (!(IN_GAME_MAIN_CAMERA.GameType != GameType.Multiplayer || !photonView.isMine || obj7.GetComponent<HERO>().HasDied()))
                                {
                                    ...
                                    obj7.GetComponent<HERO>().photonView.RPC(Rpc.Die, PhotonTargets.All, objArray5);
                                }
                            }
                        }
                        else
                        {
                            ...
                            if (obj8 != null)
                            {
                                ...
                                else if (!(IN_GAME_MAIN_CAMERA.GameType != GameType.Multiplayer || !photonView.isMine || obj8.GetComponent<HERO>().HasDied()))
                                {
                                    ...
                                    obj8.GetComponent<HERO>().photonView.RPC(Rpc.Die, PhotonTargets.All, objArray6);
                                }
                            }
                        }
                    }
                    ...
                    if (this.attackAnimation == "throw")
                    {
                        ...
                        {
                            this.attacked = true;
                            Transform t = this.baseTransform.Find("Amarture/Core/Controller_Body/hip/spine/chest/shoulder_R/upper_arm_R/forearm_R/hand_R/hand_R_001");
                            if (IN_GAME_MAIN_CAMERA.GameType == GameType.Multiplayer && photonView.isMine)
                            {
                                this.throwRock = PhotonNetwork.Instantiate("FX/rockThrow", t.position, t.rotation, 0);
                            }
                            else
                            {
                                this.throwRock = (GameObject) Instantiate(Resources.Load("FX/rockThrow"), t.position, t.rotation);
                            }
                            ...
                            this.throwRock.transform.parent = t;
                            if (IN_GAME_MAIN_CAMERA.GameType == GameType.Multiplayer && photonView.isMine)
                            {
                                ...
                                this.throwRock.GetPhotonView().RPC(Rpc.Initialize, PhotonTargets.Others, objArray7);
                            }
                        }
                        ...
                        if (this.baseAnimation["attack_" + this.attackAnimation].normalizedTime >= 1f)
                        {
                            ...
                            if (obj10 != null)
                            {
                                ...
                                else if (!(IN_GAME_MAIN_CAMERA.GameType != GameType.Multiplayer || !photonView.isMine || obj10.GetComponent<HERO>().HasDied()))
                                {
                                    ...
                                    obj10.GetComponent<HERO>().photonView.RPC(Rpc.Die, PhotonTargets.All, objArray8);
                                }
                                if (this.abnormalType == AbnormalType.Crawler)
                                {
                    ...
                    else if (this.attackAnimation == "jumper_1" || this.attackAnimation == "crawler_jump_1")
                    {
                        if (this.baseAnimation["attack_" + this.attackAnimation].normalizedTime >= 1f && this.grounded)
                        {
                            ...
                            this.currentCamera.GetComponent<IN_GAME_MAIN_CAMERA>().startShake(num23, num23);
                        }
                    }
                                ...
                                if (obj13 != null)
                                {
                                    ...
                                    else if (IN_GAME_MAIN_CAMERA.GameType == GameType.Multiplayer && photonView.isMine)
                                    {
                                        ...
                                        else if (!obj13.GetComponent<HERO>().HasDied())
                                        {
                                            ...
                                            obj13.GetComponent<HERO>().photonView.RPC(Rpc.Die, PhotonTargets.All, objArray9);
                                        }
                                    }
                                }
    ...
    public void updateCollider()
    {
        if (this.colliderEnabled)
        {
            ...
            {
                foreach (Collider c in this.baseColliders)
                {
                    if (c != null)
                        c.enabled = false;
                }
