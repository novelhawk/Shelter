...
public class TITAN : Photon.MonoBehaviour
{
    ...
    public void dieBlowFunc(Vector3 attacker, float hitPauseTime)
    {
        if (!this.hasDie)
        {
            ...
            if (photonView.isMine)
            {
                ...
                {
                    this.grabbedTarget.GetPhotonView().RPC(Rpc.netUngrabbed, PhotonTargets.All, new object[0]);
                }
                if (this.nonAI)
                {
    ...
    private void eatSet(GameObject grabTarget)
    {
        if (IN_GAME_MAIN_CAMERA.GameType != GameType.Singleplayer && (IN_GAME_MAIN_CAMERA.GameType != GameType.Multiplayer || !photonView.isMine) || !grabTarget.GetComponent<HERO>().IsGrabbed)
        {
            ...
            if (IN_GAME_MAIN_CAMERA.GameType == GameType.Multiplayer && photonView.isMine)
            {
                photonView.RPC(Rpc.GrabRight, PhotonTargets.Others, new object[0]);
    ...
    private void eatSetL(GameObject grabTarget)
    {
        if (IN_GAME_MAIN_CAMERA.GameType != GameType.Singleplayer && (IN_GAME_MAIN_CAMERA.GameType != GameType.Multiplayer || !photonView.isMine) || !grabTarget.GetComponent<HERO>().IsGrabbed)
        {
            ...
            if (IN_GAME_MAIN_CAMERA.GameType == GameType.Multiplayer && photonView.isMine)
            {
                photonView.RPC(Rpc.GrabLeft, PhotonTargets.Others, new object[0]);
                grabTarget.GetPhotonView().RPC(Rpc.PlayAnimation, PhotonTargets.All, "grabbed");
    ...
    public void loadskin()
    {
        ...
        if ((IN_GAME_MAIN_CAMERA.GameType == GameType.Singleplayer || photonView.isMine) && ModuleManager.Enabled(nameof(ModuleEnableSkins)))
        {
            ...
            if (IN_GAME_MAIN_CAMERA.GameType == GameType.Singleplayer)
                ...
                photonView.RPC(Rpc.LoadSkin, PhotonTargets.AllBuffered, body, eye);
        }
    }
