...
using Mod.Discord.Exceptions;
using Mod.Discord.Helper;
using Mod.Discord.IO;
using Mod.Discord.Logging;
using Mod.Discord.Message;
using Mod.Discord.RPC.Commands;
...
namespace Mod.Discord.RPC
{
	...
	public class RpcConnection : IDisposable
	{
		...
		public RpcConnection(string applicationID, int processID, int targetPipe, INamedPipeClient client)
		{
			...
			File.WriteAllBytes("discord-rpc.log", new byte[0]);
			Logger = new FileLogger("discord-rpc.log");
		...
		private void ProcessFrame(EventPayload response)
		{
			...
			{
				if (response.Command == Payload.Command.Dispatch && response.Event.HasValue && response.Event.Value == ServerEvent.Ready)
				{
					...
					//Prepare the object
					ReadyMessage ready = response.Data.ReadyMessage;
					lock (l_config)
					{
			...
			if (State == RpcState.Connected)
			{
				switch(response.Command)
				{
					...
					case Payload.Command.SetActivity:
						...
						{
							RichPresenceResponse rp = response.Data.RichPresenceResponse;
							EnqueueMessage(new PresenceMessage(rp));
						}
						break;

					case Payload.Command.Unsubscribe:
					case Payload.Command.Subscribe:
...
//						var evt = response.Data.GetValue("evt").ToObject<ServerEvent>(serializer);
					...
					case Payload.Command.SendActivityJoinInvite:
						Logger.Info("Got invite response ack.");
						break;

					case Payload.Command.CloseActivityJoinRequest:
		...
		{
			if (response.Command != Payload.Command.Dispatch) return;
			...
			switch(response.Event.Value)
			{
				...
				case ServerEvent.ActivitySpectate:
					var spectate = new SpectateMessage();
					EnqueueMessage(spectate);
					break;

				case ServerEvent.ActivityJoin:
					var join = new JoinMessage {Secret = response.Data.Secret};
					EnqueueMessage(join);
					break;

				case ServerEvent.ActivityJoinRequest:
					var request = new JoinRequestMessage {User = response.Data.User};
